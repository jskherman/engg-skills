...
def main():
    ...
    try:
        ...
        path=write_json(result_envelope(skill='steam_tables_iapws', inputs=vars(a), results=res, warnings=warnings, sources=sources), a.output)
        print(f'Success. JSON written to: {path}'); return 0
    except Exception as e:
        write_json(result_envelope(skill='steam_tables_iapws', inputs=vars(a), results={'error':str(e)}, ok=False, warnings=['Do not invent steam properties; use validated IAPWS methods or provide trusted data.']), a.output)
