...
def main():
    ...
    try:
        ...
        path=write_json(result_envelope(skill='design_of_experiments', inputs=vars(a), results={'run_count':len(runs), 'runs':runs}, assumptions=['Generated plan only; confirm feasibility, blocking, randomization, and safety before execution.']), a.output); print(f'Success. JSON written to: {path}'); return 0
    except Exception as e:
        write_json(result_envelope(skill='design_of_experiments', inputs=vars(a), results={'error':str(e)}, ok=False), a.output); print(f'Error. JSON written to: {a.output}', file=sys.stderr); return 1
