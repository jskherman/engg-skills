...
def main() -> int:
    ...
    try:
        ...
        path=write_json(result_envelope(skill='material-energy-balances', inputs=vars(args), results=res, assumptions=assumptions), args.output); print(f'Success. JSON written to: {path}'); return 0
    except Exception as e:
        write_json(result_envelope(skill='material-energy-balances', inputs=vars(args), results={'error':str(e)}, ok=False), args.output); print(f'Error. JSON written to: {args.output}', file=sys.stderr); return 1
