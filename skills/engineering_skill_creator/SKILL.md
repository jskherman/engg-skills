...
description: >-
  ...
  frontmatter with `Use when... Don't use for...` description, common
  mistakes catalogue, workflow checklist, fallback strategies, JSON output
...
- Editing the shared `engg_skills_common` library (that is developer code,
...
- Workflows MUST be numbered checklists, not prose paragraphs.
...
1. Pick a folder name in `snake_case` (e.g. `vle_flash_calculations`).
...
5. If the script needs a new pure-Python or thin-wrapper helper, add it to
   `skills/engg_skills_common/engg_skills_common/<module>.py` and import
