...
def main():
    ...
    try:
        ...
        path=write_json(result_envelope(skill='statistical_process_control', inputs=vars(a), results=res, assumptions=['Preliminary SPC calculation; verify rational subgrouping and measurement system.']), a.output); print(f'Success. JSON written to: {path}'); return 0
    except Exception as e:
        write_json(result_envelope(skill='statistical_process_control', inputs=vars(a), results={'error':str(e)}, ok=False), a.output); print(f'Error. JSON written to: {a.output}', file=sys.stderr); return 1
